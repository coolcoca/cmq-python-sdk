...
class Topic:
    ...
    def delete(self):
        ''' delete opic '''
        params = {"topicName": self.topicName}
    ...
    def list_subscription(self, searchWord="", limit=-1, offset=""):
        '''列出Topic的订阅
            ...
            @type offset: string
            @param offset: list_subscription的起始位置，
                上次list_subscription返回的next_offset

            @rtype: tuple
            @return: subscriptionURL的列表和下次list subscription的起始位置;
                如果所有subscription都list出来，next_offset为"".
        '''
        params = {
            'topicName': self.topicName,
        }
    ...
    def __str__(self):
        msg_info = {
            ...
            "createTime": time.strftime(
                "%Y/%m/%d %H:%M:%S", time.localtime(self.createTime)),
            "lastModifyTime": time.strftime(
                "%Y/%m/%d %H:%M:%S", time.localtime(self.lastModifyTime)),
            "maxMsgSize": self.maxMsgSize,
            "msgRetentionSeconds": self.msgRetentionSeconds,
            "filterType": self.filterType}
        return "\n".join(
            ["%s: %s" % (k.ljust(30), v) for k, v in msg_info.items()])
